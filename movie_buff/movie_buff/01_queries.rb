...
def harrison_ford
  ...
  #find movies where actor.name ="Harrison Ford" and NOT castings.ord = 1
end
...
def biggest_cast
  ...
  Movie.select(:id, :title).joins(:actors).group(:id).order('COUNT(castings.actor_id) DESC').limit(3) 
